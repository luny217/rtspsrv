...
RB_HEAD(epoll_sock_data_tree, epoll_sock_data_s);

#define EPOLL_IOCP_KEY_HANDLE   0
#define EPOLL_IOCP_KEY_SIGNAL   1
/* State associated with a epoll handle. */
struct epoll_port_data_s {
  HANDLE iocp;
  SOCKET peer_sockets[ARRAY_COUNT(AFD_PROVIDER_IDS)];
  struct epoll_sock_data_tree sock_data_tree;
...
static void epoll_sock_list_add(epoll_port_data_t *port_data, epoll_sock_data_t *sock_data) {
...
static void epoll_sock_list_del(epoll_port_data_t *port_data, epoll_sock_data_t *sock_data) {
...
int epoll_ctl(epoll_t port_handle, int op, SOCKET sock,
  ...
  switch (op) {
    case EPOLL_CTL_ADD: {
      ...
      epoll_sock_list_add(port_data, sock_data);
    ...
    case EPOLL_CTL_MOD: {
      ...
      if (ev->events & EPOLL__EVENT_MASK & ~sock_data->submitted_events) {
        if (sock_data->free_op == NULL) {
          epoll_op_t* op = (epoll_op_t*)malloc(sizeof *op);
          if (op == NULL) {
            SetLastError(ERROR_OUTOFMEMORY);
            return -1;
          }
    ...
    case EPOLL_CTL_DEL: {
      ...
      if (sock_data->flags & EPOLL__SOCK_LISTED) {
        epoll_sock_list_del(port_data, sock_data);
        sock_data->flags &= ~EPOLL__SOCK_LISTED;
      }
...
int epoll_wait(epoll_t port_handle, struct epoll_event* events, int maxevents,
  ...
  do {
    ...
    while (port_data->attn_list != NULL) {
	  ...
	  //printf("[i:%d fd:%p]sock = %d, flag = %d\n", ii++, sock_data, sock_data->sock, sock_data->flags);
      ...
      if (sock_data->registered_events & EPOLL__EVENT_MASK &
        ...
        if (r) {
          events[0].data = sock_data->user_data;
          events[0].events = EPOLLERR;
          //LeaveCriticalSection(&port_data->op_lock);
    ...
    for (i = 0; i < count; i++) {
      ...
      if (entries[i].lpCompletionKey == EPOLL_IOCP_KEY_SIGNAL)
        continue;
      op = CONTAINING_RECORD(entries[i].lpOverlapped, epoll_op_t, overlapped);
