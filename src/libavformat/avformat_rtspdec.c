                            ...
                            int * rbuflen)
{
    ...
    *rbuflen      = 0;
    ...
    do
    {
        ret = ffurl_read_complete(rt->rtsp_hd, (unsigned char *)(rbuf + idx), 1);
        if (ret <= 0)
                           ...
                           const char * extracontent, uint16_t seq)
{
    ...
    ffurl_write(rt->rtsp_hd, (unsigned char *)message, strlen(message));
...
{
    RTSPState * rt = s->priv_data;
    unsigned char * session_id = (unsigned char *)rt->session_id;
    if (!session_id[0])
    {
        av_log(s, AV_LOG_WARNING, "There is no session-id at the moment\n");
        return 0;
    }
    if (strcmp((char *)session_id, request->session_id))
    {
...
int ff_rtsp_setup_input_streams(AVFormatContext * s, RTSPMessageHeader * reply)
{
    ...
    if (!content && reply->status_code == 401)/* luny 20150716 */
    {
        return AVERROR_AUTH_FAIL;
    }
...
static int rtsp_listen(AVFormatContext * s)
{
    ...
    ff_url_join(rt->control_uri, sizeof(rt->control_uri), "rtsp", NULL, host, port, "%s", path);

    if (port < 0)
        port = RTSP_DEFAULT_PORT;

    /* Create TCP connection */
    ff_url_join(tcpname, sizeof(tcpname), "tcp", NULL, host, port, "?listen&listen_timeout=%d", rt->initial_timeout * 1000);

    if (ret = ffurl_open(&rt->rtsp_hd, tcpname, AVIO_FLAG_READ_WRITE, &s->interrupt_callback, NULL))
    {
...
int rtsp_read_header(AVFormatContext * s)
{
    RTSPState * rt = s->priv_data;
    int ret;
                            ...
                            uint8_t * buf, int buf_size)
{
    ...
    int id, len, i, ret;
    RTSPStream * rtsp_st;
    /* rt->rtsp_hd->rw_timeout = 1000;//luny */
...
redo:
    ...
    if (ret != 3)
    {
         av_log(s, AV_LOG_DEBUG, "rtsp read 3bytes error = %d\n", ret);
        return ret;
    }
    id  = buf[0];
    len = AV_RB16(buf + 1);
#ifdef DEBUG_PACKET_FLOW
     av_log(s, AV_LOG_DEBUG, "Network received id=%d len=%d\n", id, len);
#endif
    ...
    if (ret != len)
    {
         av_log(s, AV_LOG_ERROR, " rtsp read complete error = %d %d\n ", ret, len);
        return ret;
    }
...
#if 0
int rtsp_read_packet(AVFormatContext * s, AVPacket * pkt);
...
retry:
    ...
    if (!(rt->rtsp_flags & RTSP_FLAG_LISTEN))
    {
        ...
        /*if ((av_gettime() - rt->last_cmd_time) / 1000000 >= rt->timeout / 2 || rt->auth_state.stale)*/
        if ((av_gettime() - rt->last_cmd_time) / 1000000 > 30 || rt->auth_state.stale)
        {
