...
const AVOption ff_rtsp_options[] =
{
    ...
    /* { "user-agent", "override User-Agent header", OFFSET(user_agent), AV_OPT_TYPE_STRING, {.str = "RTSP-CLIENT"}, 0, 0, DEC }, */
    { NULL },
};
                           ...
                           int letter, const char * buf)
{
    ...
    switch (letter)
    {
        ...
        case 'm':
            ...
            else if (!strcmp(st_type, "application"))   /* for Speco */
            {
                codec_type = AVMEDIA_TYPE_DATA;
            }
            else if (!strcmp(st_type, "text"))    /* luny 2015.09.11 tvt */
            {
                codec_type = AVMEDIA_TYPE_DATA;
            }
                ...
                if (s->nb_streams > 0)
                {
                    ...
                    if (rtsp_st->dynamic_handler &&
                        ...
                        rtsp_st->dynamic_handler->parse_sdp_a_line(s,
                                ...
                                rtsp_st->dynamic_protocol_context, buf);/* parse_h264_sdp_line */
                }
            }
            break;
    }
...
static void rtsp_parse_transport(RTSPMessageHeader * reply, const char * p)
{
    ...
    for (;;)
    {
        ...
        while (*p != '\0' && *p != ',')
        {
            ...
            while (*p != ';' && *p != '\0' && *p != ',')
                p++;
            /*有ipc使用错误的逗号分隔，忽略逗号是否有问题 luny@2015.04.28*/
            if (*p == ';' || *p == ',')
                p++;
        }
        if (*p == ',')
...
start:
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (ret == 0) /*The remote host invokes shutdown or close*/
                return AVERROR_EOF;
            else if (ret != 1) /*ETIMEDOUT is mostly seen*/
                return ret;
            if (ch == '\n')
                break;
            if (ch == '$')
    ...
    if (request)
    {
        ...
        {
            av_base64_encode(base64buf, (int)sizeof(base64buf), (const uint8_t *)buf, strlen(buf));
            ptr = base64buf;
        }
        ffurl_write(rt->rtsp_hd_out, (const unsigned char *)ptr, strlen(ptr));
        ...
        int send_content_length)
{
    ...
    if (rt->control_transport == RTSP_MODE_TUNNEL)
    {
        av_base64_encode(base64buf, sizeof(base64buf), (const uint8_t *)buf, strlen(buf));
        out_buf = base64buf;
    }

    av_dlog(s, "Sending:\n%s--\n", buf);

    ffurl_write(rt->rtsp_hd_out, (const uint8_t *)out_buf, strlen(out_buf));
    if (send_content_length > 0 && send_content)
    {
        if (rt->control_transport == RTSP_MODE_TUNNEL)
        {
...
retry:
    ...
    if (reply->status_code > 400)
    {
        av_log(s, AV_LOG_ERROR, "method %s failed: %d%s\n",
               ...
               reply->status_code,
               reply->reason);
        av_log(s, AV_LOG_DEBUG, "aaa%s\n", rt->last_reply);
    }

    return 0;
}

int ff_rtsp_make_setup_request(AVFormatContext * s, const char * host, int port, int lower_transport)
{
    RTSPState * rt = s->priv_data;
...
have_port:
        ...
        if (reply->transports[0].lower_transport != (enum RTSPLowerTransport)lower_transport)
        {
...
redirect:
    ...
    else
    {
        ...
        if (ffurl_open(&rt->rtsp_hd, tcpname, AVIO_FLAG_READ_WRITE, &s->interrupt_callback, NULL) < 0)
        {
            err = AVERROR(EIO);
            goto fail;
        }
    ...
    do
    {
        err = ff_rtsp_make_setup_request(s, host, port, RTSP_LOWER_TRANSPORT_TCP);
        if (err < 0)
            goto fail;
    }
    while (err);
...
redo:
    ...
    if (!rt->recvbuf)
    {
        ...
        /*
         * shutup valgrind error message:
         * "conditional jump or move depends on uninitialized value(s)"
         */
        rt->recvbuf = av_mallocz(RECVBUF_SIZE);
#else
    ...
    switch (rt->lower_transport)
    {
        ...
        case RTSP_LOWER_TRANSPORT_TCP:
            ...
            {
                ff_rtp_check_and_send_back_rr_custom(rtsp_st->transport_priv, rt->rtsp_hd, NULL, len);
            }
            break;
#if 0
        case RTSP_LOWER_TRANSPORT_UDP:
    ...
    if (rt->transport == RTSP_TRANSPORT_RTP)
    {
        ...
        if (rtsp_st->feedback)
        {
            ...
            /* ff_rtp_send_rtcp_feedback(rtsp_st->transport_priv, rt->rtsp_hd, pb);//luny */
        }
        if (ret < 0)
        {
...
static int sdp_probe(AVProbeData * p1)
{
    ...
    while (p < p_end && *p != '\0')
    {
        if (p + sizeof("c=IN IP") - 1 < p_end &&
                av_strstart((const char *)p, "c=IN IP", NULL))
