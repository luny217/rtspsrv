...
void * n_slice_alloc(uint32_t mem_size)
{
    void  * mem;
#ifdef _RTSPSRV_TEST
    mem = av_malloc(mem_size);
#else
    mem = mem_manage_malloc(COMP_RTSPSRV, mem_size);
#endif
    return mem;
}
...
void n_slice_free_chain_with_offset(uint32_t mem_size, void * mem_chain, uint32_t next_offset)
{
    ...
    while (slice)
    {
        ...
        av_free(current);
#else
        mem_manage_free(COMP_RTSPSRV, current);
#endif
...
void n_slice_free1(uint32_t mem_size, void * mem_block)
{
    if (mem_block)
    {
        ...
        av_free(mem_block);
#else
        mem_manage_free(COMP_RTSPSRV, mem_block);
#endif
...
void n_free(void * mem)
{
    if (mem)
    {
        ...
        av_free(mem);
#else
        mem_manage_free(COMP_RTSPSRV, mem);
#endif
...
static inline n_dlist_t * _n_dlist_remove_link(n_dlist_t * list, n_dlist_t * link)
{
    ...
    if (link->prev)
    {
        if (link->prev->next == link)
            ...
            rsrv_log(AV_LOG_ERROR, "corrupted double-linked list detected");
    }
    if (link->next)
    {
