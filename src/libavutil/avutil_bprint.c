...
#endif
#include "avutil_avassert.h"
#include "avutil_avstring.h"
#include "avutil_bprint.h"
#include "avutil_common.h"
#include "avutil_error.h"
#include "avutil_mem.h"
...
#else
    new_str = mem_manage_realloc(COMP_RTSPSRV, old_str, new_size);
#endif    
    if (!new_str)
...
void av_bprintf(AVBPrint * buf, const char * fmt, ...)
{
    ...
    while (1)
    {
        ...
        if (extra_len <= 0)
            return;
        /*Start of luny on 2017-7-14 9:27 1.0*/
        if ((unsigned)extra_len < room)
            /*End of luny on 2017-7-14 9:27 1.0*/
            break;
...
void av_vbprintf(AVBPrint * buf, const char * fmt, va_list vl_arg)
{
    ...
    while (1)
    {
        ...
        if (extra_len <= 0)
            return;
        /*Start of luny on 2017-7-14 9:26 1.0*/
        if ((unsigned)extra_len < room)
            /*End of luny on 2017-7-14 9:26 1.0*/
            break;
        if (av_bprint_alloc(buf, extra_len))
            break;
    }
...
int av_bprint_finalize(AVBPrint * buf, char ** ret_str)
{
...
#else
            str = mem_manage_realloc(COMP_RTSPSRV, buf->str, real_size);
#endif            
            if (!str)
                str = buf->str;
            buf->str = NULL;
...
#else
            str = mem_manage_malloc(COMP_RTSPSRV, real_size);
#endif
    ...
    else
    {
        if (av_bprint_is_allocated(buf))
            ...
            av_freep(&buf->str);
#else
            mem_manage_free(COMP_RTSPSRV, buf->str);
#endif
    }
    buf->size = real_size;
    return ret;
