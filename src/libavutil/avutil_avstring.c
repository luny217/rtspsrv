...
size_t av_strlcatf(char * dst, size_t size, const char * fmt, ...)
{
    ...
    va_start(vl, fmt);
    /*Start of luny on 2017-7-14 9:25 1.0*/
    len += vsnprintf(dst + len, size > (size_t)len ? size - len : 0, fmt, vl);
    /*End of luny on 2017-7-14 9:25 1.0*/
    va_end(vl);
...
char * av_asprintf(const char * fmt, ...)
{
    ...
    p = av_malloc(len + 1);
#else
    p = mem_manage_malloc(COMP_RTSPSRV, len + 1);
#endif
    ...
    if (len < 0)
        ...
        av_freep(&p);
#else
        mem_manage_free(COMP_RTSPSRV, p);
#endif
...
#else
    char * str = mem_manage_malloc(COMP_RTSPSRV, 16);
#endif
    if (str)
        snprintf(str, 16, "%f", d);
    return str;
...
#else
    char * out = mem_manage_malloc(COMP_RTSPSRV, strlen(*buf) + 1);
#endif
